...
## Status
Unity mobile-first scaffold is in-repo, with initial `MainTable` scene layout, first-pass touch aim + swipe-shot input scripts, a foundational turn state machine, and centralized pool physics tuning helpers now landed; next up are pocket handling, foul specifics, and HUD polish.
