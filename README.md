...
## Status
Unity mobile-first scaffold is in-repo, with initial `MainTable` scene layout, first-pass touch aim + swipe-shot input scripts, a foundational turn state machine, centralized pool physics tuning helpers, baseline pocket-detection/rules signaling scripts, first-pass remote sensor cue integration (aim/shot mapping + instant touch fallback), and a scenario-driven EditMode gameplay test harness for turn/foul/win transitions now landed; next up are calibration UX, stability filtering, foul specifics, full pocket integration in-scene, and HUD polish.
